...
async function processQuery(query) {
  ...
  try {
    ...
    if (match && query.length > 4000) {
      ...
      whereClause = false;
      await executeQueryToDatabase(query);
...
document.querySelector('#downloadExcel').addEventListener('click', function() {
  ...
  console.log('@@@@@@ ws and wb sheet done');
  ...
  console.log('@@@@@@@ blob prepared '  );

    const url = URL.createObjectURL(blob);
    console.log('@@@@@@@ url creating done');
    ...
    exportButton.innerText  = 'Download Excel';
    // Clean up
    setTimeout(function() {
...
function isDisableCancle(isDisabled){
  cancelElement.disabled = isDisabled;
}
